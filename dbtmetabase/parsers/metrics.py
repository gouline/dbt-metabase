...
class MetabaseMetricCompiler:
    def __init__(self, field_lookup: Mapping):
        """Class for interafacing with Metabase metric compiler

        Args:
            field_lookup (Mapping): This consists of metabase fields as output by `build_metadata_lookups`. This parameter is what allows the compiler to map fields to ids
        """
        ...
        """Current target is a stateful variable which holds on to the table name which helps us scope the field lookup"""
    ...
    # PACKS FIELDS -> TO INCLUDE METABASE FIELD ID DETERMINATION LOGIC
    def to_field(self, y):
        # y[0]
    ...
    def transpile_expression(self, metabase_expression):
        in_memory_result = self._parser.parseString(metabase_expression).asList()
